...
3. DB-related functionality is closely linked to the Mnemic client via namespaces (`mnemic.nodes.entity.save(node)`), not scattered across data model classes.
...
# Node operations
await mnemic.nodes.entity.save(node)
await mnemic.nodes.entity.get_by_uuid("abc-123")
await mnemic.nodes.episode.recent_episodes(reference_time, last_n=5)

# Edge operations
await mnemic.edges.entity.save(edge)
await mnemic.edges.entity.get_between_nodes(source_uuid, target_uuid)

# Transactions
async with mnemic.driver.transaction() as tx:
    await mnemic.nodes.entity.save(node1, tx=tx)
    await mnemic.nodes.entity.save(node2, tx=tx)

# High-level search (orchestration stays on client)
results = await mnemic.search(query, ...)
```
...
```python
# mnemic/driver/query_executor.py — standalone, no deps on ops or GraphDriver
...
Namespaces                     (depend on GraphDriver)
     ↑
Mnemic                       (depends on Namespaces + GraphDriver)
```
...
async with mnemic.driver.transaction() as tx:
    await mnemic.nodes.entity.save(node1, tx=tx)
    await mnemic.nodes.entity.save(node2, tx=tx)
    await mnemic.edges.entity.save(edge, tx=tx)
...
─────────                          ─────────                           ───────
mnemic.nodes.entity.save(        EntityNodeNamespace.save(           EntityNodeOperations.save(
...
class EpisodeNodeOperations(ABC):
    ...
    @abstractmethod
    async def recent_episodes(self, executor: QueryExecutor, reference_time: datetime,
...
Thin wrappers on the Mnemic client that orchestrate non-DB concerns
...
class Mnemic:
    def __init__(self, ..., graph_driver: GraphDriver | None = None, ...):
        ...
        # High-level search orchestration stays as methods on Mnemic.
...
mnemic/
  ...
  mnemic.py                        # Mnemic client with .nodes, .edges properties
...
4. Wire `Mnemic` with `self.nodes`, `self.edges`
