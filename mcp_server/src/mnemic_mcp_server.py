#!/usr/bin/env python3
"""
Mnemic MCP Server - Exposes Mnemic functionality through the Model Context Protocol (MCP)
"""
...
from mnemic.edges import EntityEdge
from mnemic.nodes import EpisodeType, EpisodicNode
from mnemic.search.search_filters import SearchFilters
from mnemic.utils.maintenance.graph_data_operations import clear_data
from mcp.server.fastmcp import FastMCP
from pydantic import BaseModel
from starlette.responses import JSONResponse

from config.schema import MnemicConfig, ServerConfig
...
# Semaphore limit for concurrent Mnemic operations.
#
...
MNEMIC_MCP_INSTRUCTIONS = """
Mnemic is a memory service for AI agents built on a knowledge graph. Mnemic performs well
with dynamic data such as user interactions, changing enterprise data, and external information.

Mnemic transforms information into a richly connected knowledge network, allowing you to 
capture relationships between concepts, entities, and information. The system organizes data as episodes 
(content snippets), nodes (entities), and facts (relationships between entities), creating a dynamic, 
queryable memory store that evolves with new information. Mnemic supports multiple data formats, including 
...
mcp = FastMCP(
    'Mnemic Agent Memory',
    instructions=MNEMIC_MCP_INSTRUCTIONS,
)

# Global services
mnemic_service: Optional['MnemicService'] = None
queue_service: QueueService | None = None

# Global client for backward compatibility
mnemic_client: Mnemic | None = None
semaphore: asyncio.Semaphore


class MnemicService:
    """Mnemic service using the unified configuration system."""

    def __init__(self, config: MnemicConfig, semaphore_limit: int = 10):
        self.config = config
        self.semaphore_limit = semaphore_limit
        self.semaphore = asyncio.Semaphore(semaphore_limit)
        self.client: Mnemic | None = None
        self.entity_types = None

    async def initialize(self) -> None:
        """Initialize the Mnemic client with factory-created components."""
        try:
            ...
            custom_types = None
            if self.config.mnemic.entity_types:
                custom_types = {}
                for entity_type in self.config.mnemic.entity_types:
            ...
            # Initialize Mnemic client with appropriate driver
            try:
                if self.config.database.provider.lower() == 'falkordb':
                    # For FalkorDB, create a FalkorDriver instance directly
                    from mnemic.driver.falkordb_driver import FalkorDriver
            ...
            logger.info('Successfully initialized Mnemic client')
            ...
            logger.info(f'Using group_id: {self.config.mnemic.group_id}')

        except Exception as e:
            logger.error(f'Failed to initialize Mnemic client: {e}')
            raise

    async def get_client(self) -> Mnemic:
        """Get the Mnemic client, initializing if necessary."""
        if self.client is None:
            await self.initialize()
        if self.client is None:
            raise RuntimeError('Failed to initialize Mnemic client')
        return self.client
...
async def add_memory(
    ...
    """
    global mnemic_service, queue_service

    if mnemic_service is None or queue_service is None:
        return ErrorResponse(error='Services not initialized')

    try:
        # Use the provided group_id or fall back to the default from config
        effective_group_id = group_id or config.mnemic.group_id
        ...
        await queue_service.remember(
            group_id=effective_group_id,
            name=name,
            content=episode_body,
            source_description=source_description,
            episode_type=episode_type,
            entity_types=mnemic_service.entity_types,
...
async def search_nodes(
    ...
    """
    global mnemic_service

    if mnemic_service is None:
        return ErrorResponse(error='Mnemic service not initialized')

    try:
        client = await mnemic_service.get_client()

        # Use the provided group_ids or fall back to the default from config if none provided
        effective_group_ids = (
            group_ids
            if group_ids is not None
            else [config.mnemic.group_id]
            if config.mnemic.group_id
            else []
        )
        ...
        from mnemic.search.search_config_recipes import NODE_HYBRID_SEARCH_RRF
...
async def get_episodes(
    ...
    try:
        ...
        from mnemic.nodes import EpisodicNode
...
async def clear_graph(group_ids: list[str] | None = None) -> SuccessResponse | ErrorResponse:
    ...
    try:
        ...
        # Use the provided group_ids or fall back to the default from config if none provided
        effective_group_ids = (
            group_ids or [config.mnemic.group_id] if config.mnemic.group_id else []
        )
...
async def get_status() -> StatusResponse:
    """Get the status of the Mnemic MCP server and database connection."""
    global mnemic_service

    if mnemic_service is None:
        return StatusResponse(status='error', message='Mnemic service not initialized')
    ...
    try:
        ...
        provider_name = mnemic_service.config.database.provider
        return StatusResponse(
            status='ok',
            message=f'Mnemic MCP server is running and connected to {provider_name} database',
        )
    except Exception as e:
        error_msg = str(e)
        ...
        return StatusResponse(
            status='error',
            message=f'Mnemic MCP server is running but database connection failed: {error_msg}',
        )
...
async def health_check(request) -> JSONResponse:
    ...
    return JSONResponse({'status': 'healthy', 'service': 'mnemic-mcp'})


async def initialize_server() -> ServerConfig:
    """Parse CLI arguments and initialize the Mnemic server configuration."""
    global config, mnemic_service, queue_service, mnemic_client, semaphore

    parser = argparse.ArgumentParser(
        description='Run the Mnemic MCP server with YAML configuration support'
    )
    ...
    parser.add_argument(
        '--destroy-graph',
        action='store_true',
        help='Destroy all Mnemic graphs on startup',
    )
    ...
    logger.info(f'  - Group ID: {config.mnemic.group_id}')
    ...
    try:
        ...
        mnemic_version = getattr(mnemic, '__version__', 'unknown')
        logger.info(f'  - Mnemic Core: {mnemic_version}')
    except Exception:
        # Check for Docker-stored version file
        version_file = Path('/app/.mnemic-version')
        if version_file.exists():
            mnemic_version = version_file.read_text().strip()
            logger.info(f'  - Mnemic Core: {mnemic_version}')
        else:
            logger.info('  - Mnemic Core: version unavailable')

    # Handle graph destruction if requested
    if hasattr(config, 'destroy_graph') and config.destroy_graph:
        logger.warning('Destroying all Mnemic graphs as requested...')
        temp_service = MnemicService(config, SEMAPHORE_LIMIT)
    ...
    # Initialize services
    mnemic_service = MnemicService(config, SEMAPHORE_LIMIT)
    queue_service = QueueService()
    await mnemic_service.initialize()

    # Set global client for backward compatibility
    mnemic_client = await mnemic_service.get_client()
    semaphore = mnemic_service.semaphore

    # Initialize queue service with the client
    await queue_service.initialize(mnemic_client)
...
def main():
    """Main function to run the Mnemic MCP server."""
    try:
        ...
        logger.error(f'Error initializing Mnemic MCP server: {str(e)}')
        raise
