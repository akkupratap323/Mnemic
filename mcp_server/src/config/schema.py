...
class MnemicAppConfig(BaseModel):
    """Mnemic-specific configuration."""
...
class MnemicConfig(BaseSettings):
    """Mnemic configuration with YAML and environment support."""

    server: ServerConfig = Field(default_factory=ServerConfig)
    llm: LLMConfig = Field(default_factory=LLMConfig)
    embedder: EmbedderConfig = Field(default_factory=EmbedderConfig)
    database: DatabaseConfig = Field(default_factory=DatabaseConfig)
    mnemic: MnemicAppConfig = Field(default_factory=MnemicAppConfig)
    ...
    def apply_cli_overrides(self, args) -> None:
        ...
        # Override Mnemic settings
        if hasattr(args, 'group_id') and args.group_id:
            self.mnemic.group_id = args.group_id
        if hasattr(args, 'user_id') and args.user_id:
            self.mnemic.user_id = args.user_id
