...
class QueueService:
    ...
    def __init__(self):
        ...
        # Store the mnemic client after initialization
        self._mnemic_client: Any = None
    ...
    async def initialize(self, mnemic_client: Any) -> None:
        """Initialize the queue service with a mnemic client.

        Args:
            mnemic_client: The mnemic client instance to use for processing episodes
        """
        self._mnemic_client = mnemic_client
        logger.info('Queue service initialized with mnemic client')
    ...
    async def remember(
        ...
        Returns:
            The position in the queue
        """
        if self._mnemic_client is None:
            raise RuntimeError('Queue service not initialized. Call initialize() first.')

        async def process_episode():
            """Process the episode using the mnemic client."""
            try:
                logger.info(f'Processing episode {uuid} for group {group_id}')

                # Process the episode using the mnemic client
                await self._mnemic_client.remember(
                    name=name,
