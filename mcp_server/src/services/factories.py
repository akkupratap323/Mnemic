...
try:
    from mnemic.driver.falkordb_driver import FalkorDriver  # noqa: F401
...
from mnemic.llm_client.config import LLMConfig as MnemicLLMConfig

# Try to import additional providers if available
try:
    from mnemic.embedder.azure_openai import AzureOpenAIEmbedderClient
...
try:
    from mnemic.embedder.gemini import GeminiEmbedder
...
try:
    from mnemic.embedder.voyage import VoyageAIEmbedder
...
try:
    from mnemic.llm_client.azure_openai_client import AzureOpenAILLMClient
...
try:
    from mnemic.llm_client.anthropic_client import AnthropicClient
...
try:
    from mnemic.llm_client.gemini_client import GeminiClient
...
try:
    from mnemic.llm_client.groq_client import GroqClient
    ...
    def create(config: LLMConfig) -> LLMClient:
        ...
        match provider:
            case 'openai':
                ...
                from mnemic.llm_client.config import LLMConfig as CoreLLMConfig
            ...
            case 'azure_openai':
                if not HAS_AZURE_LLM:
                    raise ValueError(
                        'Azure OpenAI LLM client not available in current mnemic version'
                    )
            ...
            case 'anthropic':
                if not HAS_ANTHROPIC:
                    raise ValueError(
                        'Anthropic client not available in current mnemic version'
                    )
            ...
            case 'gemini':
                if not HAS_GEMINI:
                    raise ValueError('Gemini client not available in current mnemic version')
            ...
            case 'groq':
                if not HAS_GROQ:
                    raise ValueError('Groq client not available in current mnemic version')
    ...
    def create(config: EmbedderConfig) -> EmbedderClient:
        ...
        match provider:
            case 'openai':
                ...
                from mnemic.embedder.openai import OpenAIEmbedderConfig
            ...
            case 'azure_openai':
                if not HAS_AZURE_EMBEDDER:
                    raise ValueError(
                        'Azure OpenAI embedder not available in current mnemic version'
                    )
                if not config.providers.azure_openai:
                    raise ValueError('Azure OpenAI provider configuration not found')
                azure_config = config.providers.azure_openai
            ...
            case 'gemini':
                if not HAS_GEMINI_EMBEDDER:
                    raise ValueError(
                        'Gemini embedder not available in current mnemic version'
                    )
                if not config.providers.gemini:
                    raise ValueError('Gemini provider configuration not found')
                ...
                from mnemic.embedder.gemini import GeminiEmbedderConfig
            ...
            case 'voyage':
                if not HAS_VOYAGE_EMBEDDER:
                    raise ValueError(
                        'Voyage embedder not available in current mnemic version'
                    )
                ...
                from mnemic.embedder.voyage import VoyageAIEmbedderConfig
...
class DatabaseDriverFactory:
    ...
    Note: This returns configuration dictionaries that can be passed to Mnemic(),
    ...
    def create_config(config: DatabaseConfig) -> dict:
        ...
        match provider:
            ...
            case 'falkordb':
                if not HAS_FALKOR:
                    raise ValueError(
                        'FalkorDB driver not available in current mnemic version'
                    )
