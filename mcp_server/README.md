...
Mnemic is a framework for building and querying temporally-aware knowledge graphs, specifically tailored for AI agents
operating in dynamic environments. Unlike traditional retrieval-augmented generation (RAG) methods, Mnemic
continuously integrates user interactions, structured and unstructured enterprise data, and external information into a
coherent, queryable graph. The framework supports incremental data updates, efficient retrieval, and precise historical
queries without requiring complete graph recomputation, making it suitable for developing interactive, context-aware AI
applications.

This is an experimental Model Context Protocol (MCP) server implementation for Mnemic. The MCP server exposes
Mnemic's key functionality through the MCP protocol, allowing AI assistants to interact with Mnemic's knowledge
graph capabilities.

## Features

The Mnemic MCP server provides comprehensive knowledge graph capabilities:
...
2. Install the [Mnemic prerequisites](#prerequisites).

3. Configure Claude, Cursor, or other MCP client to use [Mnemic with a `stdio` transport](#integrating-with-mcp-clients). See the client documentation on where to find their MCP configuration files.

### For Cursor and other HTTP-enabled clients

1. Change directory to the `mcp_server` directory

`cd mnemic/mcp_server`
...
Mnemic MCP Server includes built-in entity types for structured knowledge extraction. These entity types are always enabled and configured via the `entity_types` section in your `config.yaml`:
...
To run the Mnemic MCP server with the default FalkorDB setup:
...
The Mnemic MCP server can be deployed using Docker with your choice of database backend. The Dockerfile uses `uv` for package management, ensuring consistent dependency installation.

A pre-built Mnemic MCP container is available at: `mnemic/knowledge-graph-mcp`
...
To use the Mnemic MCP server with other MCP-compatible clients, configure it to connect to the server:
...
>
> Ensure that you set the full path to the `uv` binary and your Mnemic project folder.
  ...
  "mcpServers": {
    "mnemic-memory": {
      ...
      "args": [
        ...
        "--isolated",
        "--directory",
        "/Users/<user>>/dev/zep/mnemic/mcp_server",
        "--project",
        ".",
        "main.py",
        "--transport",
...
The Mnemic MCP server exposes the following tools:

- `remember`: Add an episode to the knowledge graph (supports text, JSON, and message formats)
...
- `get_status`: Get the status of the Mnemic MCP server and Neo4j connection

## Working with JSON Data

The Mnemic MCP server can process structured JSON data through the `remember` tool with `source="json"`. This
...
To integrate the Mnemic MCP Server with the Cursor IDE, follow these steps:

1. Run the Mnemic MCP server using the default HTTP transport:
...
2. Configure Cursor to connect to the Mnemic MCP server.
...
3. Add the Mnemic rules to Cursor's User Rules. See [cursor_rules.md](cursor_rules.md) for details.

4. Kick off an agent session in Cursor.

The integration enables AI assistants in Cursor to maintain persistent memory through Mnemic's knowledge graph
capabilities.

## Integrating with Claude Desktop (Docker MCP Server)

The Mnemic MCP Server uses HTTP transport (at endpoint `/mcp/`). Claude Desktop does not natively support HTTP transport, so you'll need to use a gateway like `mcp-remote`.

1.  **Run the Mnemic MCP server**:
...
3.  **Configure Claude Desktop**:
    ...
    ```json
    {
      "mcpServers": {
        "mnemic-memory": {
          ...
          "args": [
            "mcp-remote",
            "http://localhost:8000/mcp/" // The Mnemic server's HTTP endpoint
          ]
        }
      }
    }
    ...
    If you already have an `mcpServers` entry, add `mnemic-memory` (or your chosen name) as a new key within it.
...
The Mnemic MCP server uses the Mnemic core library, which includes anonymous telemetry collection. When you initialize the Mnemic MCP server, anonymous usage statistics are collected to help improve the framework.

### What's Collected

- Anonymous identifier and system information (OS, Python version)
- Mnemic version and configuration choices (LLM provider, database backend, embedder type)
...
```bash
export MNEMIC_TELEMETRY_ENABLED=false
```

Or add it to your `.env` file:

```
MNEMIC_TELEMETRY_ENABLED=false
```

For complete details about what's collected and why, see the [Telemetry section in the main Mnemic README](../README.md#telemetry).

## License

This project is licensed under the same license as the parent Mnemic project.
