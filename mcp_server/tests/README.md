# Mnemic MCP Server Integration Tests

This directory contains a comprehensive integration test suite for the Mnemic MCP Server using the official Python MCP SDK.

## Overview

The test suite is designed to thoroughly test all aspects of the Mnemic MCP server with special consideration for LLM inference latency and system performance.
...
```python
from test_fixtures import mnemic_test_client

async with mnemic_test_client(database="falkordb") as (session, group_id):
