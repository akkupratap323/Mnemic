...
from config.schema import MnemicConfig
...
def test_llm_factory(config: MnemicConfig):
...
def test_embedder_factory(config: MnemicConfig):
...
async def test_database_factory(config: MnemicConfig):
    ...
    if config.database.provider == 'neo4j' and config.database.providers.neo4j:
        try:
            ...
            # Test actual connection would require initializing Mnemic
            ...
            try:
                ...
                await mnemic.driver.client.verify_connectivity()
                print('  ✓ Successfully connected to Neo4j')
                await mnemic.driver.client.close()
            except Exception as e:
...
def test_cli_override():
    ...
    print(f'  - Database provider: {config.database.provider}')
    print(f'  - Group ID: {config.mnemic.group_id}')
    print(f'  - User ID: {config.mnemic.user_id}')
