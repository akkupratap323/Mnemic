"""
Shared test fixtures and utilities for Mnemic MCP integration tests.
"""
...
@pytest.fixture
async def mock_mnemic_client():
    """Provide a Mnemic client with mocked LLM."""
    async with mnemic_test_client(use_mock_llm=True) as (session, group_id):
        yield session, group_id


@pytest.fixture
async def mnemic_client():
    """Provide a real Mnemic client."""
    async with mnemic_test_client(use_mock_llm=False) as (session, group_id):
        yield session, group_id
