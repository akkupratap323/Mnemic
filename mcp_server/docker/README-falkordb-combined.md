# FalkorDB + Mnemic MCP Server Combined Image

This Docker setup bundles FalkorDB (graph database) and the Mnemic MCP Server into a single container image for simplified deployment.
...
- **Mnemic MCP Server**: Knowledge graph API on port 8000
...
docker run -d \
  ...
  -e MNEMIC_GROUP_ID=main \
  -v falkordb_data:/var/lib/falkordb/data \
  mnemic/mnemic-falkordb:latest
```
...
### Build with Specific Mnemic Version

```bash
MNEMIC_CORE_VERSION=0.22.0 docker compose -f docker/docker-compose-falkordb-combined.yml build
```

### Build Arguments

- `MNEMIC_CORE_VERSION`: Version of mnemic package (default: 0.22.0)
...
- `MNEMIC_GROUP_ID`: Namespace for graph data (default: "main")
...
- `/var/log/mnemic`: MCP server and FalkorDB Browser logs
...
# Only container logs
docker compose -f docker/docker-compose-falkordb-combined.yml logs -f mnemic-falkordb
```
...
docker compose -f docker/docker-compose-falkordb-combined.yml exec mnemic-falkordb redis-cli ping
...
```bash
docker compose -f docker/docker-compose-falkordb-combined.yml logs mnemic-falkordb
```
