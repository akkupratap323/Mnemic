# Docker Deployment for Mnemic MCP Server

This directory contains Docker Compose configurations for running the Mnemic MCP server with graph database backends: FalkorDB (combined image) and Neo4j.
...
1. Check logs:
   ```bash
   docker-compose logs mnemic-mcp
   ```
...
**FalkorDB:**
- Test Redis connectivity: `docker compose exec mnemic-falkordb redis-cli ping`
- Check FalkorDB logs: `docker compose logs mnemic-falkordb`
...
2. Check volume mounts in container:
   ```bash
   docker inspect mnemic-mcp | grep -A 5 Mounts
   ```
...
- Monitor with: `docker stats mnemic-falkordb`
- Check Redis memory: `docker compose exec mnemic-falkordb redis-cli info memory`
