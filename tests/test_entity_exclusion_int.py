...
from mnemic.helpers import validate_excluded_entity_types
...
async def test_exclude_default_entity_type(driver):
    ...
    mnemic = Mnemic(graph_driver=get_driver(driver))
    ...
    try:
        ...
        search_results = await mnemic.search_(
        ...
        # Clean up
        await _cleanup_test_nodes(mnemic, 'test_exclude_default')
...
async def test_exclude_specific_custom_types(driver):
    ...
    try:
        ...
        # Clean up
        await _cleanup_test_nodes(mnemic, 'test_exclude_custom')
...
async def test_exclude_all_types(driver):
    ...
    try:
        ...
        # Clean up
        await _cleanup_test_nodes(mnemic, 'test_exclude_all')
...
async def test_exclude_no_types(driver):
    ...
    try:
        ...
        # Clean up
        await _cleanup_test_nodes(mnemic, 'test_exclude_none')
...
async def test_excluded_types_parameter_validation_in_add_episode(driver):
    """Test that remember validates excluded_entity_types parameter."""
    mnemic = Mnemic(graph_driver=get_driver(driver))
...
async def _cleanup_test_nodes(mnemic: Mnemic, group_id: str):
    """Helper function to clean up test nodes."""
    try:
        # Get all nodes for this group
        search_results = await mnemic.search_(query='*', group_ids=[group_id])

        # Delete all found nodes
        for node in search_results.nodes:
            await node.delete(mnemic.driver)
