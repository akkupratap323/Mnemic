...
from mnemic.core import AddEpisodeResults
from mnemic.helpers import semaphore_gather
from mnemic.llm_client import LLMConfig, OpenAIClient
from mnemic.nodes import EpisodeType
from mnemic.prompts import prompt_library
from mnemic.prompts.eval import EvalAddEpisodeResults
from tests.test_mnemic_int import NEO4J_URI, NEO4j_PASSWORD, NEO4j_USER
...
async def build_graph(
    group_id_suffix: str, multi_session_count: int, session_length: int, mnemic: Mnemic
...
async def build_baseline_graph(multi_session_count: int, session_length: int):
    ...
    mnemic = Mnemic(NEO4J_URI, NEO4j_USER, NEO4j_PASSWORD, llm_client=llm_client)

    add_episode_results, _ = await build_graph(
        'baseline', multi_session_count, session_length, mnemic
    )
...
async def eval_graph(multi_session_count: int, session_length: int, llm_client=None) -> float:
    ...
    add_episode_results, add_episode_context = await build_graph(
        'candidate', multi_session_count, session_length, mnemic
    )
