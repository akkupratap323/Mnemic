...
from mnemic.utils.maintenance.attribute_utils import (
...
def test_env_var_overrides_default(monkeypatch):
    monkeypatch.setenv('MNEMIC_ATTRIBUTE_MAX_LENGTH', '50')
    long_value = 'x' * 100
...
def test_env_var_invalid_falls_back_to_default(monkeypatch, caplog):
    monkeypatch.setenv('MNEMIC_ATTRIBUTE_MAX_LENGTH', 'not-a-number')
    short_value = 'x' * 100
    with caplog.at_level(logging.WARNING):
...
def test_env_var_non_positive_or_blank_falls_back(monkeypatch, bad_value):
    monkeypatch.setenv('MNEMIC_ATTRIBUTE_MAX_LENGTH', bad_value)
...
def test_invalid_env_warning_dedups_across_calls(monkeypatch, caplog):
    from mnemic.utils.maintenance import attribute_utils

    monkeypatch.setattr(attribute_utils, '_warned_invalid_env', set())
    monkeypatch.setenv('MNEMIC_ATTRIBUTE_MAX_LENGTH', 'garbage')
    ...
    invalid_warnings = [
        ...
        if rec.levelno == logging.WARNING and 'MNEMIC_ATTRIBUTE_MAX_LENGTH' in rec.message
    ]
