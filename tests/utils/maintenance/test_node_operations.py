...
def _make_clients():
    ...
    clients = MnemicClients.model_construct(  # bypass validation to allow test doubles
        driver=driver,
        embedder=embedder,
...
async def test_resolve_nodes_exact_match_skips_llm(monkeypatch):
    ...
    monkeypatch.setattr(
        'mnemic.utils.maintenance.node_operations._semantic_candidate_search',
...
async def test_resolve_with_llm_candidate_attributes_cannot_overwrite_candidate_id(monkeypatch):
    ...
    monkeypatch.setattr(
        'mnemic.utils.maintenance.node_operations.prompt_library.dedupe_nodes.nodes',
        fake_prompt_nodes,
    )
