...
from mnemic.core import Mnemic
from mnemic.llm_client import LLMClient
from mnemic.nodes import EntityNode
...
async def test_add_triplet_merges_attributes(
    ...
    with (
        patch('mnemic.core.search') as mock_search,
        patch('mnemic.core.resolve_extracted_edge') as mock_resolve_edge,
    ):
        mock_search.return_value = Mock(edges=[])
        mock_resolve_edge.return_value = (edge, [], [])

        await mnemic.remember_fact(user_source, edge, user_target)
...
async def test_add_triplet_with_new_nodes_no_uuid(
    ...
    """Test remember_fact with nodes that don't have UUIDs (will be resolved)."""
    mnemic = Mnemic(
        graph_driver=graph_driver,
        llm_client=mock_llm_client,
        embedder=mock_embedder,
    ...
    with patch('mnemic.core.search') as mock_search:
        mock_search.return_value = Mock(edges=[])
        with patch('mnemic.core.resolve_extracted_edge') as mock_resolve_edge:
            mock_resolve_edge.return_value = (edge, [], [])

            result = await mnemic.remember_fact(user_source, edge, user_target)
...
async def test_add_triplet_edge_uuid_with_different_nodes_creates_new_edge(
    ...
    with (
        ...
        result = await mnemic.remember_fact(alice, new_edge_with_same_uuid, charlie)
...
async def test_add_triplet_edge_uuid_with_same_nodes_updates_edge(
    ...
    with (
        ...
        result = await mnemic.remember_fact(alice, updated_edge, bob)
