"""
Modifications Copyright 2026, Abishek (Mnemic project).
Based on Graphiti, Copyright 2024, Zep Software, Inc.
...
from mnemic.cross_encoder.client import CrossEncoderClient
from mnemic.cross_encoder.openai_reranker_client import OpenAIRerankerClient
from mnemic.decorators import handle_multiple_group_ids
from mnemic.driver.driver import GraphDriver
from mnemic.driver.neo4j_driver import Neo4jDriver
from mnemic.edges import (
    CommunityEdge,
    Edge,
    EntityEdge,
    EpisodicEdge,
...
from mnemic.embedder import EmbedderClient, OpenAIEmbedder
from mnemic.errors import EdgeNotFoundError, NodeNotFoundError
from mnemic.types import MnemicClients
from mnemic.helpers import (
    get_default_group_id,
    semaphore_gather,
    validate_excluded_entity_types,
    validate_group_id,
)
from mnemic.llm_client import LLMClient, OpenAIClient
from mnemic.namespaces import EdgeNamespace, NodeNamespace
from mnemic.nodes import (
    CommunityNode,
    EntityNode,
    EpisodeType,
    EpisodicNode,
...
from mnemic.prompts.lib import prompt_library
from mnemic.prompts.summarize_sagas import SagaSummary
from mnemic.search.search import SearchConfig, search
from mnemic.search.search_config import DEFAULT_SEARCH_LIMIT, SearchResults
from mnemic.search.search_config_recipes import (
    COMBINED_HYBRID_SEARCH_CROSS_ENCODER,
    EDGE_HYBRID_SEARCH_NODE_DISTANCE,
    EDGE_HYBRID_SEARCH_RRF,
)
from mnemic.search.search_filters import SearchFilters
from mnemic.search.search_utils import (
    RELEVANT_SCHEMA_LIMIT,
    get_mentioned_nodes,
)
from mnemic.tracer import Tracer, create_tracer
from mnemic.utils.bulk_utils import (
    RawEpisode,
...
from mnemic.utils.datetime_utils import utc_now
from mnemic.utils.maintenance.community_operations import (
    build_communities,
    remove_communities,
    update_community,
)
from mnemic.utils.maintenance.edge_operations import (
    build_episodic_edges,
    extract_edges,
    resolve_extracted_edge,
    resolve_extracted_edges,
)
from mnemic.utils.maintenance.graph_data_operations import (
    EPISODE_WINDOW_LEN,
    recent_episodes,
)
from mnemic.utils.maintenance.node_operations import (
    extract_attributes_from_nodes,
    extract_nodes,
    resolve_extracted_nodes,
)
from mnemic.utils.ontology_utils.entity_types_utils import validate_entity_types
from mnemic.utils.text_utils import MAX_SUMMARY_CHARS
...
class Mnemic:
    def __init__(
        ...
        trace_span_prefix: str = 'mnemic',
    ):
        """
        Initialize a Mnemic instance.
        ...
        max_coroutines : int | None, optional
            ...
            If not set, the Mnemic default is used.
        ...
        trace_span_prefix : str, optional
            Prefix to prepend to all span names. Defaults to 'mnemic'.
        ...
        Mnemic if you're using the default OpenAIClient.
        """
        ...
        # Initialize namespace API (mnemic.nodes.entity.save(), etc.)
    ...
    async def close(self):
        ...
        It should be called when the Mnemic instance is no longer needed or
        ...
        Example:
            mnemic = Mnemic(uri, user, password)
            try:
                # Use mnemic...
            finally:
                mnemic.close()
    ...
    async def _get_or_create_saga(
        ...
        """
        from mnemic.helpers import parse_db_date
    ...
    async def recent_episodes(
        ...
        Notes
        -----
        The actual retrieval is performed by the `recent_episodes` function
        from the `mnemic.utils` module, unless a saga is specified.
        """
        if driver is None:
        ...
        if driver.graph_operations_interface:
            try:
                return await driver.graph_operations_interface.recent_episodes(
                    driver, reference_time, last_n, group_ids, source, saga
                )
            except NotImplementedError:
                pass

        return await recent_episodes(driver, reference_time, last_n, group_ids, source, saga)
    ...
    async def remember(
        ...
        Example using FastAPI background tasks:
            ...
            async def add_episode_endpoint(episode_data: EpisodeData):
                background_tasks.add_task(mnemic.remember, **episode_data.dict())
        ...
        with self.tracer.start_span('remember') as span:
            try:
                # Retrieve previous episodes for context
                previous_episodes = (
                    await self.recent_episodes(
                        reference_time,
                ...
                logger.info(f'Completed remember in {(end - start) * 1000} ms')
    ...
    async def remember_many(
        ...
        ``resolve_extracted_edges`` just like in ``remember``, and any invalidated
        edges are persisted alongside the newly resolved ones.
        """
        with self.tracer.start_span('remember_many') as bulk_span:
            ...
            try:
                ...
                logger.info(f'Completed remember_many in {(end - start) * 1000} ms')
    ...
    async def forget_episode(self, episode_uuid: str):
