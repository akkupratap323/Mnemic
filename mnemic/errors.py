...
class MnemicError(Exception):
    """Base exception class for Mnemic Core."""


class EdgeNotFoundError(MnemicError):
...
class EdgesNotFoundError(MnemicError):
...
class GroupsEdgesNotFoundError(MnemicError):
...
class GroupsNodesNotFoundError(MnemicError):
...
class NodeNotFoundError(MnemicError):
...
class SearchRerankerError(MnemicError):
    """Raised when a node is not found."""
...
class EntityTypeValidationError(MnemicError):
...
class GroupIdValidationError(MnemicError):
...
class NodeLabelValidationError(MnemicError, ValueError):
