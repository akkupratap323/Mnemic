...
This example demonstrates how to use Mnemic with Azure OpenAI and Neo4j to build a knowledge graph.
...
The example shows how to configure Mnemic to use Azure OpenAI with the OpenAI v1 API:
...
# Initialize Mnemic with custom clients
mnemic = Mnemic(
    neo4j_uri,
    neo4j_user,
    neo4j_password,
...
Episodes are the primary units of information in Mnemic. They can be:
- **Text**: Raw text content (e.g., transcripts, documents)
- **JSON**: Structured data with key-value pairs

### Hybrid Search

Mnemic combines multiple search strategies:
...
- Explore other search recipes in `mnemic/search/search_config_recipes.py`
...
- `examples/quickstart/` - Basic Mnemic usage with OpenAI
