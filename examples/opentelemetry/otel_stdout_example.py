...
def setup_otel_stdout_tracing():
    ...
    resource = Resource(attributes={'service.name': 'mnemic-example'})
...
async def main():
    ...
    mnemic = Mnemic(
        graph_driver=kuzu_driver, tracer=otel_tracer, trace_span_prefix='mnemic.example'
    )
    ...
    try:
        ...
        results = await mnemic.search('What positions has Gavin Newsom held?')
