...
-----------------------------------------
This example demonstrates how Mnemic handles different types of content:
...
async def main():
    ...
    try:
        ...
        # Mnemic will process it in a single LLM call.
        #################################################
        ...
        # Mnemic will automatically chunk it for
        ...
        results = await mnemic.search('Q4 planning meeting participants')
        ...
        results = await mnemic.search('AWS S3 costs')
        print(f'Found {len(results)} results')
        for r in results[:3]:
            print(f'  - {r.fact}')
        ...
        results = await mnemic.search('ECS Fargate recommendations')
        print(f'Found {len(results)} results')
        for r in results[:3]:
            print(f'  - {r.fact}')
