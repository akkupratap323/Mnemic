...
This example demonstrates using [GLiNER2](https://github.com/fastino-ai/GLiNER2) as a hybrid LLM client for Mnemic. GLiNER2 handles entity extraction (NER) locally on CPU, while a general-purpose LLM client handles edge/fact extraction, deduplication, summarization, and other reasoning tasks.
...
```bash
# Install mnemic with the gliner2 extra
pip install mnemic[gliner2]
...
The example uses Google Gemini (`gemini-2.5-flash-lite`) for the LLM and embeddings, but `GLiNER2Client` accepts any Mnemic `LLMClient`. To swap providers, replace `GeminiClient` and `GeminiEmbedder` with the equivalent from another provider:

- `mnemic.llm_client.openai_client.OpenAIClient`
- `mnemic.llm_client.anthropic_client.AnthropicClient`
- `mnemic.llm_client.groq_client.GroqClient`
- `mnemic.embedder.openai.OpenAIEmbedder`
- `mnemic.embedder.voyage.VoyageAIEmbedder`
